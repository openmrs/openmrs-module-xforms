...
/**
 * Parses an xforms document and puts text in all nodes referencing an itext block for a given
 * language. The first language in the itext block is taken to be the current or default one.
 * 
 * @author daniel
 */
public class ItextParser {
	...
	/**
	 * Gets the itext id from a given itext expression as represented by the ref attribute of a given
	 * node.
	 * 
	...
	private static boolean isBindNode(Element node) {
		return (node.getNodeName().equalsIgnoreCase("bind") || node.getNodeName().equalsIgnoreCase("xf:bind"));
	}
}
