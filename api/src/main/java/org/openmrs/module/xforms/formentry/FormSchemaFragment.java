...
/**
 * Convenience class for generating various fragments of an XML Schema for OpenMRS form definitions.
 * 
 * @author Burke Mamlin
 * @version 1.0
...
public class FormSchemaFragment {
	 ...
	 * 
	 * @param namespace the namespace for this form
	 * @return XML Schema header
	 */
	public static String header(Form form) {
		return "<?xml version=\"1.0\"?>\n" + "<xs:schema xmlns:xs=\"http://www.w3.org/2001/XMLSchema\"\n"
		        + "           xmlns:openmrs=\"" + FormEntryWrapper.getFormSchemaNamespace(form) + "\"\n"
	 ...
	 * 
	 * @param formId form's internal identifier
	 * @param formName form's name
	 * @param formVersion version of form
	...
	public static String closeForm(Form form) {
		return "    </xs:sequence>\n" + "    <xs:attribute name=\"id\" type=\"xs:positiveInteger\" fixed=\""
		        + form.getFormId() + "\" use=\"required\" />\n"
		        + "    <xs:attribute name=\"name\" type=\"xs:string\" fixed=\"" + StringEscapeUtils.escapeXml(form.getName())
	 ...
	 * 
	 * @param token tag name for simple concept element
	 * @param concept OpenMRS concept referred to by this element
	 * @param xsType XML Schema datatype
	 * @param required <code>true</code> if element is required
	 * @return XML Schema fragment for a simple concept
	 */
	public static String simpleConcept(String token, Concept concept, String xsType, boolean required, Locale locale) {
	 ...
	 * 
	 * @param token tag name for numeric concept element
	 * @param concept OpenMRS concept referred to by this element
	 * @param required <code>true</code> if element is required
	 * @param minInclusive minimum value for valid response (inclusive)
	 * @param maxInclusive maximum value for valid response (inclusive)
	 * @param precise <code>true</code> for real numbers, <code>false</code> for integers
	 * @return XML fragment for a numeric concept element
	 */
	public static String numericConcept(String token, ConceptNumeric concept, boolean required, Locale locale) {
		...
		if (!skipBounds) {
			xml += "<xs:simpleType name=\"" + token + "_restricted_type\">\n" + "  <xs:restriction base=\""
			        ...
			        + (minInclusive == null ? ""
			                : "    <xs:minInclusive value=\"" + numericToString(minInclusive, precise) + "\" />\n")
			        + (maxInclusive == null ? ""
			                : "    <xs:maxInclusive value=\"" + numericToString(maxInclusive, precise) + "\" />\n")
			        + "  </xs:restriction>\n" + "</xs:simpleType>\n";
		}
		xml += "<xs:complexType name=\"" + token + "\">\n" + "  <xs:sequence>\n"
		        ...
		        + "    <xs:element name=\"time\" type=\"xs:time\" nillable=\"true\" minOccurs=\"0\" />\n"
		        + (skipBounds
		                ? "    <xs:element name=\"value\" type=\"" + (precise ? "xs:float" : "xs:int") + "\" nillable=\""
		                        + (required ? "0" : "1") + "\" />\n"
		                : "    <xs:element name=\"value\" type=\"" + token + "_restricted_type\" nillable=\""
		                        + (required ? "0" : "1") + "\" />\n")
		        + "  </xs:sequence>\n"
		        + "  <xs:attribute name=\"openmrs_concept\" type=\"xs:string\" use=\"required\" fixed=\""
		        + StringEscapeUtils.escapeXml(FormUtil.conceptToString(concept, locale)) + "\" />\n"
	...
	/**
	 * Returns a string representing a given number. The string is formatted based upon the necessary
	 * precision (real vs. integer).
	 * 
	 * @param value numeric value to be rendered
	 * @param precise <code>true</code> for real numbers, <code>false</code> for integers
	 ...
	 * 
	 * @param token tagname of element
	 * @param concept OpenMRS concept referred to by this element
	 * @param answerList valid answers that are assumed to be non retired
	 * @param required <code>true</code> if element is required
	 * @param locale current context's locale
	 * @return XML fragment representing a single-selection element
	 */
	public static String selectSingle(String token, Concept concept, Collection<ConceptAnswer> answerList, boolean required,
	        Locale locale) {
		...
		for (ConceptAnswer answer : answerList) {
			...
			if (answer.getAnswerConcept().getConceptClass().getConceptClassId().equals(HL7Constants.CLASS_DRUG)
				...
				xml += "          <xs:enumeration value=\""
				        + StringEscapeUtils.escapeXml(FormUtil.conceptToString(answer.getAnswerConcept(), locale)) + "^"
				        + FormUtil.drugToString(answer.getAnswerDrug()) + "\" /> <!-- " + answerDrugName + " -->\n";
			} else {
				xml += "          <xs:enumeration value=\""
				        + StringEscapeUtils.escapeXml(FormUtil.conceptToString(answer.getAnswerConcept(), locale))
				        + "\" /> <!-- " + answerConceptName + " -->\n";
	 ...
	 * 
	 * @param token tagname for multiple-selection element
	 * @param concept OpenMRS concept referred to by this element
	 * @param answerList valid answers
	 * @param locale context's locale
	 * @return XML fragment representing a multiple-selection element
	 */
	public static String selectMultiple(String token, Concept concept, Collection<ConceptAnswer> answerList, Locale locale) {
		String xml = "<xs:complexType name=\"" + token + "\">\n" + "  <xs:sequence>\n"
		        + "    <xs:element name=\"date\" type=\"xs:date\" nillable=\"true\" minOccurs=\"0\" />\n"
		        + "    <xs:element name=\"time\" type=\"xs:time\" nillable=\"true\" minOccurs=\"0\" />\n";
		...
		for (ConceptAnswer answer : answerList) {
			...
			if (answerDrug != null) {
				...
				xml += "    <xs:element name=\"" + FormUtil.getXmlToken(answerDrugName)
				        + "\" default=\"false\" nillable=\"true\">\n" + "      <xs:complexType>\n"
				        + "        <xs:simpleContent>\n" + "          <xs:extension base=\"xs:boolean\">\n"
				        + "            <xs:attribute name=\"openmrs_concept\" type=\"xs:string\" use=\"required\" fixed=\""
				        + StringEscapeUtils.escapeXml(FormUtil.conceptToString(answerConcept, locale)) + "^"
				...
				xml += "    <xs:element name=\"" + FormUtil.getXmlToken(answerConceptName)
				        + "\" default=\"false\" nillable=\"true\">\n" + "      <xs:complexType>\n"
				        + "        <xs:simpleContent>\n" + "          <xs:extension base=\"xs:boolean\">\n"
				        + "            <xs:attribute name=\"openmrs_concept\" type=\"xs:string\" use=\"required\" fixed=\""
				        + StringEscapeUtils.escapeXml(FormUtil.conceptToString(answerConcept, locale)) + "\" />\n"
