...
/**
 * This class implements the task which handles SMS connection requests for xforms together with the
 * corresponding user and patient downloads.
 * 
 * @author Daniel
 * @version 1.0
 */
public class XformsSMSTask extends AbstractTask {
	...
	public void execute() {
		...
		try {
			...
			server = new XformsSMSServer(modemId, modemPort, Integer.valueOf(msgDestPort), Integer.valueOf(msgSrcPort),
			        Integer.valueOf(modemBaudRate), modemManufacturer, modemModel);
			server.start();
