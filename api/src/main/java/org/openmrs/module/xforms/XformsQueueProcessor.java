...
/**
 * Processes Xforms Queue entries. When the processing is successful, the queue entry is submitted
 * to the FormEntry Queue. For unsuccessful processing, the queue entry is put in the Xforms error
 * folder.
 * 
...
public class XformsQueueProcessor {
		...
		try {
			...
			for (File file : queueDir.listFiles()) {
				try {
					...
					log.error(Context.getMessageSourceService().getMessage("xforms.problemProcessingXform")
	 ...
	 * @throws Exception
	 */
	private void saveNewPatientEncounterIfAny(Patient patient, Element root, String pathName, boolean propagateErrors)
	        throws Exception {
		...
		setNodeValue(formNode, XformBuilder.NODE_ENCOUNTER_LOCATION_ID,
		    patient.getIdentifiers().iterator().next().getLocation().getLocationId().toString());
	 ...
	 * @param pathName the full path and name of file form which this xform model has been read. null
	 *            can be passed if the form does not come from a file.
	 */
	public void processXForm(String xml, String pathName, boolean propagateErrors, HttpServletRequest request)
	        throws Exception {
	 ...
	 * @param archive
	 */
	private void submitXForm(Document doc, String xml, String pathName, boolean archive, boolean propagateErrors)
	        throws Exception {
		String xmlOriginal = xml;
		try {
	 ...
	 * @param queuePathName - the path and name of this file in the queue. If you dont supply this, a
	 *            new radom file is created in this folder, else the a file with the same name as the
	 *            queued one is created in this folder.
	 */
	private String saveForm(String xml, File folder, String queuePathName) {
		...
		if (queuePathName == null)
			...
			pathName = folder.getAbsolutePath() + File.separatorChar
			        + queuePathName.substring(queuePathName.lastIndexOf(File.separatorChar) + 1);
	...
	private String saveFormInError(String xml, String queuePathName, Exception exception) {
		...
		Context.getService(XformsService.class).sendStacktraceToAdminByEmail(
		    Context.getMessageSourceService().getMessage("xforms.problemFailedProcessForm") + errorPath, exception);
	 ...
	 */
	private Patient saveNewPatient(Element root, User creator, boolean propagateErrors, HttpServletRequest request)
	        throws Exception {
		...
		if (val != null && val.length() > 0)
			try {
				pt.setBirthdate(XformsUtil.fromSubmitString2Date(val));
			}
			catch (Exception e) {
				log.error(val, e);
			}
		...
		if (patients == null || patients.size() == 0) {
			...
			String message = Context.getMessageSourceService().getMessage("xforms.problemPatientExists")
			        + identifier.getIdentifier() + Context.getMessageSourceService().getMessage("xforms.accepted");
			log.error(message);
	...
	private void addPersonAttributes(Patient pt, Element root, XformsService xformsService, User creator) throws Exception {
		...
		List<PersonAttributeType> personAttributeTypes = Context.getPersonService()
		        .getPersonAttributeTypes(PERSON_TYPE.PERSON, null);
	...
	private void addPersonRepeatAttributeValues(Patient pt, Node repeatNode, String attributeId, XformsService xformsService,
	        int displayOrder) {
		if (repeatNode == null)
			return;
	...
	/**
	 * Check if we are to reject forms for patients considered new when they already exist, by virture
	 * of patient identifier.
	 * 
	 * @return true if we are to reject, else false.
	 */
	private boolean rejectExistingPatientCreation() {
		String reject = Context.getAdministrationService().getGlobalProperty(
		    XformConstants.GLOBAL_PROP_KEY_REJECT_EXIST_PATIENT_CREATE, XformConstants.DEFAULT_REJECT_EXIST_PATIENT_CREATE);
	...
	/**
	 * Converts xforms multiple select answer values to the format expected by the openmrs form model.
	 * 
	...
	private void setMultipleSelectNodeValues(Node parentNode) {
		...
		for (int i = 0; i < nodes.getLength(); i++) {
			...
			if (name.equalsIgnoreCase(XformBuilder.NODE_DATE) || name.equalsIgnoreCase(XformBuilder.NODE_TIME)
			        || name.equalsIgnoreCase(XformBuilder.NODE_VALUE)
			        || name.equalsIgnoreCase(XformBuilder.NODE_XFORMS_VALUE))
				continue;
	...
	private boolean isMultipleSelectNode(Node node) {
		...
		if (attributes != null) {
			...
			if (attributes.getNamedItem(XformBuilder.ATTRIBUTE_OPENMRS_CONCEPT) != null && multipleValue != null
			        && multipleValue.getNodeValue().equals("1"))
				multipSelect = true;
		}
	...
	private void fillPatientIdIfMissing(Document doc) throws Exception {
		...
		if (patients != null && patients.size() == 1) {
			DOMUtil.setElementValue(doc.getDocumentElement(), XformBuilder.NODE_PATIENT_PATIENT_ID,
			    patients.get(0).getPatientId().toString());
			return;
		}
		...
		if (identifierType == null || identifierType.trim().length() == 0) {
			...
			if (identifierType == null || identifierType.trim().length() == 0) {
				identifierType = Context.getAdministrationService()
				        .getGlobalProperty("xforms.new_patient_identifier_type_id", null);
				if (identifierType == null || identifierType.trim().length() == 0)
		...
		patient.setBirthdateEstimated(
		    "true".equals(DOMUtil.getElementValue(doc, XformBuilder.NODE_PATIENT_BIRTH_DATE_ESTIMATED)));
		
		addPersonAttributes(patient, doc.getDocumentElement(), Context.getService(XformsService.class),
		    patient.getCreator());
	...
	private void saveComplexObsValue(Element element, boolean useValueNode) throws Exception {
		...
		File file = OpenmrsUtil.getOutFile(XformsUtil.getXformsComplexObsDir(path), new Date(),
	...
	private void addOtherIdentifiers(Patient pt, Element root, User creator, PatientService patientService,
	        boolean propagateErrors, HttpServletRequest request) throws Exception {
	...
	private void addOtherIdentifier(Element root, User creator, Patient pt, PatientService patientService,
	        boolean propagateErrors, HttpServletRequest request) throws Exception {
		...
		if (nodes == null || nodes.getLength() == 0)
			reportError(Context.getMessageSourceService().getMessage("xforms.formShouldHaveID") + identifierValue,
			    propagateErrors, request);
		
		String identifierTypeId = nodes.item(0).getTextContent();
		if (identifierTypeId == null || identifierTypeId.trim().length() == 0)
			reportError(Context.getMessageSourceService().getMessage("xforms.selectIdentifierType") + identifierValue,
			    propagateErrors, request);
		
		//Look for identifier location id
		nodes = root.getElementsByTagName(XformBuilder.NODE_NAME_OTHER_IDENTIFIER_LOCATION_ID);
		if (nodes == null || nodes.getLength() == 0)
			reportError(Context.getMessageSourceService().getMessage("xforms.shouldHaveLocationType") + identifierValue,
			    propagateErrors, request);
		
		String identifierLocationId = nodes.item(0).getTextContent();
		if (identifierLocationId == null || identifierLocationId.trim().length() == 0)
			reportError(Context.getMessageSourceService().getMessage("xforms.selectLocationType") + identifierValue,
			    propagateErrors, request);
		...
		for (PatientIdentifier existingIdentifier : pt.getIdentifiers()) {
			if (existingIdentifier.getIdentifierType() == identifierType) {
				reportError(
				    Context.getMessageSourceService().getMessage("xforms.patientHasIdentifier") + identifierType.getName(),
				    propagateErrors, request);
			}
		}
		...
		List<Patient> patients = patientService.getPatients(identifier.getIdentifier());
		if (patients != null && patients.size() > 0) {
			reportError(Context.getMessageSourceService().getMessage("xforms.triedCreatePatientAlreadyExists")
