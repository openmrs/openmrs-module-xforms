...
public class XformPatientEdit {
	...
	public static boolean isPatientElement(Element element) {
		return (element.getName().equalsIgnoreCase(XformBuilder.NODE_PATIENT)
		        && String.valueOf(XformConstants.PATIENT_XFORM_FORM_ID)
		                .equals(element.getAttributeValue(null, (XformBuilder.ATTRIBUTE_ID))));
	}
	...
	public static Patient getEditedPatient(HttpServletRequest request, Element rootNode) throws Exception {
		...
		patient.setBirthdateEstimated(
		    "true".equals(XformBuilder.getNodeValue(rootNode, XformBuilder.NODE_BIRTH_DATE_ESTIMATED)));
		...
		Location location = Context.getLocationService()
		        .getLocation(Integer.parseInt(XformBuilder.getNodeValue(rootNode, XformBuilder.NODE_LOCATION_ID)));
	...
	private static void savePersonAddresses(Patient patient, Element rootNode) throws Exception {
		...
		pa.setAddress1(XformBuilder.getNodeValue(rootNode,
		    XformBuilder.NODE_NAME_PREFIX_PERSON_ADDRESS + XformBuilder.NODE_NAME_ADDRESS1));
		pa.setAddress2(XformBuilder.getNodeValue(rootNode,
		    XformBuilder.NODE_NAME_PREFIX_PERSON_ADDRESS + XformBuilder.NODE_NAME_ADDRESS2));
		pa.setCityVillage(XformBuilder.getNodeValue(rootNode,
		    XformBuilder.NODE_NAME_PREFIX_PERSON_ADDRESS + XformBuilder.NODE_NAME_CITY_VILLAGE));
		pa.setStateProvince(XformBuilder.getNodeValue(rootNode,
		    XformBuilder.NODE_NAME_PREFIX_PERSON_ADDRESS + XformBuilder.NODE_NAME_STATE_PROVINCE));
		pa.setPostalCode(XformBuilder.getNodeValue(rootNode,
		    XformBuilder.NODE_NAME_PREFIX_PERSON_ADDRESS + XformBuilder.NODE_NAME_POSTAL_CODE));
		pa.setCountry(XformBuilder.getNodeValue(rootNode,
		    XformBuilder.NODE_NAME_PREFIX_PERSON_ADDRESS + XformBuilder.NODE_NAME_COUNTRY));
		pa.setLatitude(XformBuilder.getNodeValue(rootNode,
		    XformBuilder.NODE_NAME_PREFIX_PERSON_ADDRESS + XformBuilder.NODE_NAME_LATITUDE));
		pa.setLongitude(XformBuilder.getNodeValue(rootNode,
		    XformBuilder.NODE_NAME_PREFIX_PERSON_ADDRESS + XformBuilder.NODE_NAME_LONGITUDE));
		pa.setCountyDistrict(XformBuilder.getNodeValue(rootNode,
		    XformBuilder.NODE_NAME_PREFIX_PERSON_ADDRESS + XformBuilder.NODE_NAME_COUNTY_DISTRICT));
		pa.setAddress3(XformBuilder.getNodeValue(rootNode,
		    XformBuilder.NODE_NAME_PREFIX_PERSON_ADDRESS + XformBuilder.NODE_NAME_NEIGHBORHOOD_CELL));
		pa.setAddress6(XformBuilder.getNodeValue(rootNode,
		    XformBuilder.NODE_NAME_PREFIX_PERSON_ADDRESS + XformBuilder.NODE_NAME_REGION));
		pa.setAddress5(XformBuilder.getNodeValue(rootNode,
		    XformBuilder.NODE_NAME_PREFIX_PERSON_ADDRESS + XformBuilder.NODE_NAME_SUBREGION));
		pa.setAddress4(XformBuilder.getNodeValue(rootNode,
