...
public interface XformsService {
	...
	/**
	 * Saves an Xform in the database. If it already exists, it will be overwritten, else will create a
	 * new one.
	 * 
	 * @param xform
	 */
	...
	/**
	 * Saves an xform's xslt in the database. If it already exists, it will be overwritten, else will
	 * create a new one.
	 * 
	 ...
	 * 
	 * @param formId - id of the form that owns the XForm to retrieve
	 * @param createNewIfNonExistant - set to true to create a new xform if none exists in the database,
	 *            else returns null.
	 * @return XForm associated with the form
	 */
	@Transactional(readOnly = true)
	...
	/**
	 * Gets a new XForm for the given form. This Xform is created on the fly basing on the current form
	 * schema and template, instead of using an xform stored in the database which may not match with
	 * the current schema and template.
	 * 
	 * @param formId - id of the form that owns the XForm to retrieve
	 * @return XForm associated with the form
	 */
	...
	/**
	 * Sends a stacktrace of the given exception to admin's e-mail, if it is defined under
	 * {@link XformConstants#GLOBAL_PROP_KEY_ADMIN_EMAIL}. It uses server mail properties defined under
