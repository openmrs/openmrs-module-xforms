...
/**
 * Convenience class for generating various fragments of an XML template for OpenMRS forms.
 * 
 * @author Burke Mamlin
 * @version 1.0
...
public class FormXmlTemplateFragment {
	...
	public static String header(Form form, String url) {
		return "<?xml version=\"1.0\"?>\n" + "<?mso-infoPathSolution name=\"" + encodeURL(form.getName()) + "\" href=\""
		        + url + "\" solutionVersion=\"" + FormEntryWrapper.getSolutionVersion(form)
	...
	public static String openForm(Form form, String namespace, boolean includeDefaultScripts) {
		return "<form id=\"" + form.getFormId() + "\" name=\"" + StringEscapeUtils.escapeXml(form.getName()) + "\" "
		        + "version=\"" + form.getVersion() + "\" " + "xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" "
		        + "xmlns:openmrs=\"" + namespace + "\" "
		        + "xmlns:xd=\"http://schemas.microsoft.com/office/infopath/2003\">\n" + "  <header>\n" + "    <enterer>"
		        + (includeDefaultScripts ? "$!{enterer}" : "") + "</enterer>\n" + "    <date_entered>"
		        + (includeDefaultScripts ? "$!{dateEntered}" : "") + "</date_entered>\n" + "    <session>"
		        + (includeDefaultScripts ? "$!{sessionId}" : "") + "</session>\n" + "    <uid>"
