...
public class DOMUtil {
	...
	/**
	 * Checks if a document is a create patient one. One which collected bio data about a patient.
	 * 
	 * @param doc - the document.
	...
	public static boolean isPatientElementDoc(Element element) {
		return (element.getNodeName().equalsIgnoreCase(XformBuilder.NODE_PATIENT) && String
		        .valueOf(XformConstants.PATIENT_XFORM_FORM_ID).equals(element.getAttribute(XformBuilder.ATTRIBUTE_ID)));
	}
	...
	public static boolean isEncounterElementDoc(Element element) {
		return (element.getNodeName().equalsIgnoreCase(XformBuilder.NODE_FORM)
		        && element.getAttribute(XformBuilder.ATTRIBUTE_NAME) != null
		        && element.getAttribute(XformBuilder.ATTRIBUTE_ID) != null);
	}
