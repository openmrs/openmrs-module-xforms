...
public class XformObsEdit {
	...
	public static String getMultSelObsNodeName(Element parent, Obs obs, List<String> nonCopyAttributes) {
		return XformBuilder.getElementByAttributePrefix(parent, "openmrs_concept", obs.getValueCoded().getConceptId() + "^",
		    false, "obsId", true, nonCopyAttributes).getName();
	}
	...
	public static void fillObs(HttpServletRequest request, Document doc, Integer encounterId, String xml) throws Exception {
		...
		if (encounter.getLocation() != null) {
			XformBuilder.setNodeValue(doc, XformBuilder.NODE_ENCOUNTER_LOCATION_ID,
			    encounter.getLocation().getLocationId().toString());
			XformBuilder.setNodeAttributeValue(doc, XformBuilder.NODE_ENCOUNTER_LOCATION_ID, "displayValue",
			    encounter.getLocation().getName());
		}
		
		XformBuilder.setNodeValue(doc, XformBuilder.NODE_ENCOUNTER_ENCOUNTER_DATETIME,
		    XformsUtil.encounterDateIncludesTime() ? XformsUtil.fromDateTime2SubmitString(encounter.getEncounterDatetime())
		            : XformsUtil.fromDate2SubmitString(encounter.getEncounterDatetime()));
		...
		for (Obs obs : observations) {
			...
			Element node = XformBuilder.getElementByAttributePrefix(formNode, "openmrs_concept",
			    (obsGroupId == null ? concept.getConceptId() : obs.getObsGroup().getConcept().getConceptId()) + "^",
			    (obsGroupId == null ? false : true), (obsGroupId == null ? "obsId" : "obsGroupId"), true, nonCopyAttributes);
			if (node == null)
				continue;
			...
			if (obsGroupId != null) {
				...
				node = XformBuilder.getElementByAttributePrefix(node, "openmrs_concept", concept.getConceptId() + "^", false,
				    "obsId", false, nonCopyAttributes);
				if (node == null)
					continue;
			}
			...
			else if (concept.getDatatype().getHl7Abbreviation().equals(ConceptDatatype.DATE)
			...
			else if (concept.getDatatype().getHl7Abbreviation().equals(ConceptDatatype.TIME)
			        && obs.getValueDatetime() != null)
				value = XformsUtil.fromTime2SubmitString(obs.getValueDatetime());
			else if (concept.getDatatype().getHl7Abbreviation().equals(ConceptDatatype.NUMERIC)
			...
			if ("1".equals(node.getAttributeValue(null, "multiple"))) {
				...
				if (valueNode != null) {
					...
					if (complexObs.contains(nodeName)) {
						String key = getComplexDataKey(formNode.getAttributeValue(null, "id"),
						    "/form/obs/" + node.getName() + "/value");
	...
	public static Encounter getEditedEncounter(HttpServletRequest request, Document doc, Set<Obs> obs2Void, String xml)
	        throws Exception {
		return getEditedEncounter(request, XformBuilder.getElement(doc.getRootElement(), "form"), obs2Void);
	}
	
	public static Encounter getEditedEncounter(HttpServletRequest request, Element formNode, Set<Obs> obs2Void)
	        throws Exception {
			...
			if (isMultipleSelNode(node)) {
				...
				if (valueNode != null) {
					...
					if (complexObs.contains(nodeName)) {
						...
						if (!dirtyComplexObs.contains(nodeName) && !((newValue == null || newValue.trim().length() == 0)
	...
	private static void setEncounterHeader(Encounter encounter, Element formNode) throws Exception {
		encounter.setLocation(Context.getLocationService()
		        .getLocation(Integer.valueOf(XformBuilder.getNodeValue(formNode, XformBuilder.NODE_ENCOUNTER_LOCATION_ID))));
	...
	private static void addNewObs(Element formNode, List<String> complexObs, Encounter encounter, Element obsNode,
	        Date datetime, Obs obsGroup) throws Exception {
		if (obsNode == null)
			return;
	...
	private static void addMultipleSelObs(Encounter encounter, Concept concept, Element node, Date datetime)
	        throws Exception {
	...
	private static boolean multipleSelValueContains(String nodeName, String valueName, Element node,
	        Hashtable<String, String[]> multipleSelValues) {
