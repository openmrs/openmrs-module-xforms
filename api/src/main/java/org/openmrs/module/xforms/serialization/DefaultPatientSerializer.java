...
/**
 * Provides default serialization of patients to clients. This class is used when sending a list of
 * patients to clients like mobile devices that may want to collect patient data in for instance
 * offline mode. For those who want a different serialization format for patients, just implement
 * the SerializableData interface and specify the class using the openmrs global property
 * {xforms.patientSerializer}. The jar containing this class can then be put under the
 * webapps/openmrs/web-inf/lib folder. One of the reasons one could want a different serialization
 * format is for performance by doing a more optimized and compact format. Onother reason i can
 * foresee is for non java clients who may say want the patients in xml or any other format, because
 * the default implementation assumes java clients.
 * 
 * @author Daniel
 */
