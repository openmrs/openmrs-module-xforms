...
/**
 * Provides default serialization and deserialization of users to clients. This class is used when
 * sending a list of users to clients like mobile devices that may want to collect patient data in
 * for instance offline mode. This list of users helps clients protect sensitive data on these
 * devices by requiring the same openmrs login info. For those who want a different serialization
 * format for users, just implement the SerializableData interface and specify the class using the
 * openmrs global property {xforms.userSerializer}. The jar containing this class can then be put
 * under the webapps/openmrs/web-inf/lib folder. One of the reasons one could want a different
 * serialization format is for performance by doing a more optimized and compact format. Onother
 * reason i can foresee is for non java clients who may say want the users in xml or any other
 * format, because the default implementation assumes java clients.
 * 
 * @author Daniel
 */
