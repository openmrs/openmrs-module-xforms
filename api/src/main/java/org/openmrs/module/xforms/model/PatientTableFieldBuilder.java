...
public class PatientTableFieldBuilder {
		...
		for (int i = 0; i < numOfEntries; i++) {
			...
			if (!(columnName == null || tableName == null)) {
				...
				if (XformBuilder.isUserDefinedNode(name) && !tableFieldExists(name, fields))
					fields.add(
					    new PatientTableField(fields.size() + 1, XformBuilder.getNodePath(child), tableName, columnName));
			}
	...
	public static List<PatientTableFieldValue> getPatientTableFieldValues(List<Integer> patientids,
	        List<PatientTableField> fields, XformsService xformsService) {
