...
/**
 * Schema generator for OpenMRS forms. Usage: <pre>
...
public class FormSchemaBuilder {
	...
	public synchronized String getSchema() {
		...
		for (FormField section : formStructure.get(0)) {
			...
			ComplexType ct = ComplexType.getComplexType(formStructure, schemaSections, section, sectionTag + "_section",
			    tagList);
			...
			schema.append("      <xs:element name=\"" + sectionTag + "\" type=\"" + sectionTypeTag + "\" fieldName=\""
			        + fieldName + "\" hl7Value=\"" + hl7Value + "\" />\n");
		}
		...
		for (ComplexType complexType : complexTypes) {
			...
			if (field.getFieldType().getFieldTypeId().equals(FormConstants.FIELD_TYPE_CONCEPT)) {
				...
				if (HL7Constants.simpleDatatypes.containsKey(datatype.getHl7Abbreviation()))
					schema.append(FormSchemaFragment.simpleConcept(token, concept,
					    HL7Constants.simpleDatatypes.get(datatype.getHl7Abbreviation()), required, Context.getLocale()));
					...
					schema.append(FormSchemaFragment.numericConcept(token, conceptNumeric, required, Context.getLocale()));
					...
					if (field.getSelectMultiple())
						schema.append(FormSchemaFragment.selectMultiple(token, concept, answers, Context.getLocale()));
					else
						schema.append(
						    FormSchemaFragment.selectSingle(token, concept, answers, required, Context.getLocale()));
				} else if ("ED".equals(datatype.getHl7Abbreviation()))
					schema.append(
					    FormSchemaFragment.simpleConcept(token, concept, "xs:base64Binary", required, Context.getLocale()));
			}
		}
	 ...
	 * 
	 * @param section a sequence of <code>FormField</code>s
	 * @param s the java.util.StringBuffer used for generating output
	 * @return a list of any elements encountered within the current section that have children of their
	 *         own (and, therefore, need subsequent processing); otherwise, <code>null</code>.
	 */
	private TreeSet<FormField> renderSection(TreeSet<FormField> section) {
		...
		for (FormField sectionFormField : section) {
			ComplexType sectionType = ComplexType.getComplexType(formStructure, schemaSections, sectionFormField,
			    sectionFormField.getField().getName() + "_section", tagList);
			...
			if (sectionFormFieldList != null) {
				...
				for (FormField subSectionFormField : sectionFormFieldList) {
					String elemTag = XformsUtil.getNewTag(subSectionFormField.getField().getName(), subSectionTagList);
					String elemTypeTag;
					if (formStructure.containsKey(subSectionFormField.getFormFieldId())) {
						...
						ComplexType ct = ComplexType.getComplexType(formStructure, schemaSections, subSectionFormField,
						    elemTag + "_section", tagList);
						elemTypeTag = ct.getToken();
					} else
						elemTypeTag = getNewTypeTag(subSectionFormField);
					if (subSectionFormField.getField().getFieldType().getFieldTypeId()
					        .equals(FormConstants.FIELD_TYPE_DATABASE)) {
						schema.append("    <xs:element name=\"" + elemTag + "\" ");
						if (subSectionFormField.getMinOccurs() != null)
							schema.append("minOccurs=\"" + subSectionFormField.getMinOccurs() + "\" ");
						if (subSectionFormField.getMaxOccurs() != null)
							schema.append("maxOccurs=\"" + subSectionFormField.getMaxOccurs() + "\" ");
						schema.append("nillable=\"" + (subSectionFormField.isRequired() ? "0" : "1") + "\">\n");
						schema.append("      <xs:complexType>\n");
						schema.append("        <xs:simpleContent>\n");
						schema.append("          <xs:extension base=\"" + elemTypeTag + "\">\n");
						schema.append(
						    "            <xs:attribute name=\"openmrs_table\" type=\"xs:string\" use=\"required\" fixed=\""
						            + subSectionFormField.getField().getTableName() + "\" />\n");
						schema.append(
						    "            <xs:attribute name=\"openmrs_attribute\" type=\"xs:string\" use=\"required\" fixed=\""
						            + subSectionFormField.getField().getAttributeName() + "\" />\n");
						schema.append("          </xs:extension>\n");
						schema.append("        </xs:simpleContent>\n");
						schema.append("      </xs:complexType>\n");
						schema.append("    </xs:element>\n");
					} else {
						schema.append("    <xs:element name=\"" + elemTag + "\" type=\"" + elemTypeTag + "\" ");
						if (subSectionFormField.getMinOccurs() != null)
							schema.append("minOccurs=\"" + subSectionFormField.getMinOccurs() + "\" ");
						if (subSectionFormField.getMaxOccurs() != null)
							schema.append("maxOccurs=\"" + maxOccursValue(subSectionFormField.getMaxOccurs()) + "\" ");
						schema.append("nillable=\"" + (subSectionFormField.isRequired() ? "0" : "1") + "\" />\n");
					}
				}
			}
			schema.append("  </xs:sequence>\n");
			if (sectionFormField.getField().getFieldType().getFieldTypeId().equals(FormConstants.FIELD_TYPE_CONCEPT)) {
				Concept concept = sectionFormField.getField().getConcept();
				
				schema.append("  <xs:attribute name=\"openmrs_concept\" type=\"xs:string\" use=\"required\" fixed=\""
				        + StringEscapeUtils.escapeXml(FormUtil.conceptToString(concept, Context.getLocale())) + "\" />\n");
				schema.append("  <xs:attribute name=\"openmrs_datatype\" type=\"xs:string\" use=\"required\" fixed=\""
	...
	/**
	 * Returns an appropriate representation of a maximum occurence for an XML schema.
	 * 
	 * @param n defined value for maximum occurence of a particular element
	 * @return maximum occurence or "unbounded" if maximum is not defined or is defined as -1.
	 */
	...
	/**
	 * Returns a unique tag name for field type definitions (simply adds "_type" to the end of the tag
	 * name and ensures that the tag name is unique and valid). Field is added to an array so that these
	 * complex types definitions can be rendered later.
	 * 
	 * @param f <code>FormField</code> from which to derive type
	 * @return unique tag name for datatype of given <code>FormField</code>
	 */
	private String getNewTypeTag(FormField f) {
		if (f.getField().getFieldType().getFieldTypeId().equals(FormConstants.FIELD_TYPE_CONCEPT)) {
			...
			if (i >= 0)
				...
				typeTag = XformsUtil.getNewTag(FormUtil.getXmlToken(f.getField().getName() + "_type"), tagList);
				ct.token = typeTag;
				complexTypes.add(ct);
			}
			return typeTag;
		} else if (f.getField().getFieldType().getFieldTypeId().equals(FormConstants.FIELD_TYPE_DATABASE)) {
