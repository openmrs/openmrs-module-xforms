...
public class XformConstants {
	...
	/**
	 * The name of the global property for the directory where to put xforms that are not submitted to
	 * the formentry queue because of errors.
	 */
	...
	/**
	 * The name of the global property for the name of the global property for the directory for storing
	 * complex obs.
	 */
	...
	/**
	 * The dirrectory for archiving xforms after submission to the formentry queue. The reason for
	 * archiving xforms, even after knowing that the formentry module will also archive them is that,
	 * some processing is done on these xforms to make them consumable by the formentry module, and we
	 * want users to always be able to see how the xform looked like at submission, just incase of any
	 * issues or bugs in the xforms processing.
	 */
	...
	/**
	 * XForms are linked to their correponding openmrs forms by the form_id field. For clients to create
	 * a new patient on say mobile xform clients, the new patient is also created as an xform which has
	 * to be differentiated from the rest of the forms. So as for now, we user this form_id on the
	 * assumption that no openmrs form will have it. This may not be the best way for handling this, and
	 * hence may need further thoughts, but works for now.
	 */
	...
	/**
	 * The default value for rejecting forms for patients considered new when they already exist, by
	 * virture of patient identifier.
	 */
	...
	/**
	 * The global property key for rejecting forms for patients considered new when they already exist,
	...
	/**
	 * The global property key for determining whether to use stored xforms or build new ones on the
	 * fly.
	 */
	 ...
	 * This effectively defaults to "true" - if set to "false", then the relationship nodes will *not*
	 * be added.
	 */
