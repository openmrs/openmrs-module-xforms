 ...
 * source. (i.e. those messages neither go to the archive or the error table.) Code in this class
 * has been copied from the openmrs api and modified such that hl7 processing exceptions get back to
 * the caller. Very useful feature when one submits a form in the browser and never get any error
 * message yet when deep down during hl7 processing, the form has errors.
 * 
 * @version 1.0
 */
@Transactional
public class HL7InQueueProcessor /* implements Runnable */ {
	...
	public void processHL7InQueue(HL7InQueue hl7InQueue, boolean propagateErrors) throws Exception {
		...
		try {
			...
			log.error("hl7_processor.ignore_missing_patient_non_local? " + Context.getAdministrationService()
			        .getGlobalProperty("hl7_processor.ignore_missing_patient_non_local", "false"));
			if (e.getCause() != null && e.getCause().getMessage().equals("Could not resolve patient")
			        && !hl7InQueue.getHL7Source().getName().equals("local") && Context.getAdministrationService()
			                .getGlobalProperty("hl7_processor.ignore_missing_patient_non_local", "false").equals("true")) {
				skipError = true;
			}
			if (!skipError)
