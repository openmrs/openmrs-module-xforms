...
public class XformDownloadServlet extends HttpServlet {
	...
	/**
	 * Sort out the multiple options for xformDownload. This servlet does things like the xform template
	 * with data download, the xform download.
	 */
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			if (XformConstants.REQUEST_PARAM_XFORMS.equalsIgnoreCase(target))
				...
				if (XformConstants.REQUEST_PARAM_XFORMS_LIST.equals(target)) {
					...
					if ("withurl".equalsIgnoreCase(request.getParameter("format"))) {
						...
						url += request.getContextPath()
						        + "/moduleServlet/xforms/xformDownload?target=xform&contentType=xml&excludeLayout=true&formId=";
						...
						if (xformList != null) {
							for (Object[] item : xformList)
								xml += "\n  <form url='" + url + item[0] + "'>"
								        + StringEscapeUtils.escapeXml(item[1].toString()) + "</form>";
						}
				...
				if (XformConstants.REQUEST_PARAM_CONTENT_TYPE_VALUE_XML
				        .equals(request.getParameter(XformConstants.REQUEST_PARAM_CONTENT_TYPE)))
					attachment = false;
				...
				if (request.getParameter("encounterUuid") != null) {
					Encounter encounter = Context.getEncounterService()
					        .getEncounterByUuid(request.getParameter("encounterUuid"));
					if (encounter != null) {
				...
				if (XformConstants.REQUEST_PARAM_XFORM.equalsIgnoreCase(target)) {
					...
					if (formId == 0)
						...
						doXformEntryGet(request, response, form, patient, formService, xformsService, createNew, writer);
	 ...
	 * @throws IOException
	 */
	protected void doXformGet(HttpServletRequest request, HttpServletResponse response, Form form, FormService formService,
	        XformsService xformsService, boolean createNew, boolean attachment, PrintWriter writer) throws Exception {
		...
		if (xsltKey != null)
			xformXml = XformsUtil.transformDocument(xformXml,
			    Context.getAdministrationService().getGlobalProperty(xsltKey, null));
		
		response.setCharacterEncoding(XformConstants.DEFAULT_CHARACTER_ENCODING);
		writer.print(xformXml);
	}
	
	private void doPatientXformGet(HttpServletRequest request, HttpServletResponse response, XformsService xformsService,
	        Integer formId, PrintWriter writer) throws ServletException, Exception {
		String xml = null;
	 ...
	 * @throws IOException
	 */
	protected void doXsltGet(HttpServletResponse response, Form form, XformsService xformsService, boolean createNew,
	...
	protected void doLayoutGet(HttpServletResponse response, Form form, XformsService xformsService, PrintWriter writer)
	 ...
	 * @throws IOException
	 */
	protected void doXformEntryGet(HttpServletRequest request, HttpServletResponse response, Form form, Patient patient,
	        FormService formService, XformsService xformsService, boolean createNew, PrintWriter writer) throws Exception {
		String xformXml = XformDownloadManager.getXform(formService, xformsService, form.getFormId(), createNew);
		...
		if (patientIdParam != null || patient != null) {
			if (patient == null) {
			...
			XformBuilder.setNodeAttributeValue(doc, XformBuilder.NODE_PATIENT, XformBuilder.ATTRIBUTE_UUID,
			    patient.getUuid());
	...
	protected void doPatientXformEntryGet(HttpServletRequest request, HttpServletResponse response,
	        XformsService xformsService, Integer formId, PrintWriter writer) throws Exception {
		...
		if (patientIdParam != null) {
			...
			if (patient != null) {
				...
				XformBuilder.setNodeValue(doc, XformBuilder.NODE_LOCATION_ID,
				    patient.getPatientIdentifier().getLocation().getLocationId().toString());
			...
			if ("true".equals(Context.getAdministrationService().getGlobalProperty("xforms.setDefaultProvider", "false"))) {
				//Set default provider to the logged on user of no default value is already set in the xform.
				if (user.hasRole(RoleConstants.PROVIDER)) {
					String providerId = XformBuilder.getNodeValue(doc.getRootElement(),
					    XformBuilder.NODE_ENCOUNTER_PROVIDER_ID);
					if (providerId == null || providerId.trim().length() == 0) {
						Integer id = XformsUtil.getProviderId(user);
						if (id != null) {
							XformBuilder.setNodeValue(doc, XformBuilder.NODE_ENCOUNTER_PROVIDER_ID, id.toString());
		...
		if (patient == null) {
			patient = new Patient();
			getMiniPerson(patient, request.getParameter("addName"), request.getParameter("addGender"),
			    request.getParameter("addBirthdate"), request.getParameter("addAge"));
		}
		...
		XformBuilder.setNodeValue(doc, NODE_PATIENT_BIRTH_DATE_ESTIMATED,
		    patient.getBirthdateEstimated() ? "true" : "false");
		...
		if (identifier != null) {
			...
			XformBuilder.setNodeValue(doc, XformBuilder.NODE_IDENTIFIER_TYPE_ID,
			    identifier.getIdentifierType().getPatientIdentifierTypeId().toString());
			...
			if (XformsUtil.autoGeneratePatientIdentifier()) {
				...
				if (id != null) {
					...
					XformBuilder.setNodeValue(doc, XformBuilder.NODE_IDENTIFIER_TYPE_ID,
					    patientIdentifierType.getPatientIdentifierTypeId().toString());
				}
			}
		}
	...
	private void fillPersonAddresses(Patient patient, Document doc) {
		...
		if (address.getAddress1() != null)
			XformBuilder.setNodeValue(doc, XformBuilder.NODE_NAME_PREFIX_PERSON_ADDRESS + XformBuilder.NODE_NAME_ADDRESS1,
			    address.getAddress1());
		
		if (address.getAddress2() != null)
			XformBuilder.setNodeValue(doc, XformBuilder.NODE_NAME_PREFIX_PERSON_ADDRESS + XformBuilder.NODE_NAME_ADDRESS2,
			    address.getAddress2());
		
		if (address.getCityVillage() != null)
			XformBuilder.setNodeValue(doc,
			    XformBuilder.NODE_NAME_PREFIX_PERSON_ADDRESS + XformBuilder.NODE_NAME_CITY_VILLAGE,
			    address.getCityVillage());
		
		if (address.getStateProvince() != null)
			XformBuilder.setNodeValue(doc,
			    XformBuilder.NODE_NAME_PREFIX_PERSON_ADDRESS + XformBuilder.NODE_NAME_STATE_PROVINCE,
			    address.getStateProvince());
		
		if (address.getPostalCode() != null)
			XformBuilder.setNodeValue(doc, XformBuilder.NODE_NAME_PREFIX_PERSON_ADDRESS + XformBuilder.NODE_NAME_POSTAL_CODE,
			    address.getPostalCode());
		
		if (address.getCountry() != null)
			XformBuilder.setNodeValue(doc, XformBuilder.NODE_NAME_PREFIX_PERSON_ADDRESS + XformBuilder.NODE_NAME_COUNTRY,
			    address.getCountry());
		
		if (address.getLatitude() != null)
			XformBuilder.setNodeValue(doc, XformBuilder.NODE_NAME_PREFIX_PERSON_ADDRESS + XformBuilder.NODE_NAME_LATITUDE,
			    address.getLatitude());
		
		if (address.getLongitude() != null)
			XformBuilder.setNodeValue(doc, XformBuilder.NODE_NAME_PREFIX_PERSON_ADDRESS + XformBuilder.NODE_NAME_LONGITUDE,
			    address.getLongitude());
		
		if (address.getCountyDistrict() != null)
			XformBuilder.setNodeValue(doc,
			    XformBuilder.NODE_NAME_PREFIX_PERSON_ADDRESS + XformBuilder.NODE_NAME_COUNTY_DISTRICT,
			    address.getCountyDistrict());
		
		if (address.getAddress3() != null)
			XformBuilder.setNodeValue(doc,
			    XformBuilder.NODE_NAME_PREFIX_PERSON_ADDRESS + XformBuilder.NODE_NAME_NEIGHBORHOOD_CELL,
			    address.getAddress3());
		
		if (address.getAddress6() != null)
			XformBuilder.setNodeValue(doc, XformBuilder.NODE_NAME_PREFIX_PERSON_ADDRESS + XformBuilder.NODE_NAME_REGION,
			    address.getAddress6());
		
		if (address.getAddress5() != null)
			XformBuilder.setNodeValue(doc, XformBuilder.NODE_NAME_PREFIX_PERSON_ADDRESS + XformBuilder.NODE_NAME_SUBREGION,
			    address.getAddress5());
		
		if (address.getAddress4() != null)
			XformBuilder.setNodeValue(doc,
			    XformBuilder.NODE_NAME_PREFIX_PERSON_ADDRESS + XformBuilder.NODE_NAME_TOWNSHIP_DIVISION,
			    address.getAddress4());
	}
