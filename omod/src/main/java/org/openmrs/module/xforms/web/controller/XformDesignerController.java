...
/**
 * This controller backs and saves XForm designs. As per the current version, we deal with the
 * following: For the XForm: 1 - Changing question properties like display text, readonly, required,
 * disabled, visibility etc. 2 - Order of questions. 3 - Branching or Skipping logic. 4 - For the
 * XHTML as XSL transformed from the XForm: 1 - Grouping of questions into various tabs. This avoids
 * long scrollings between questions on a form. 2 -
 */
public class XformDesignerController extends SimpleFormController {
	...
	protected Map referenceData(HttpServletRequest request, Object obj, Errors err) throws Exception {
		...
		if (formId != null && formId.trim().length() > 0) {
			...
			allowBindEdit = Context.getAdministrationService().getGlobalProperty(
			    XformConstants.GLOBAL_PROP_KEY_ALLOW_BIND_EDIT, XformConstants.DEFAULT_ALLOW_BIND_EDIT);
		} else
			map.put("formId", -1);
		...
		map.put(XformConstants.FORM_DESIGNER_KEY_LOCALE_LIST, Context.getAdministrationService()
		        .getGlobalProperty(XformConstants.GLOBAL_PROP_KEY_LOCALE_LIST, XformConstants.DEFAULT_LOCALE_LIST));
		...
		map.put("showXformsSourceTab",
		    Context.getAdministrationService().getGlobalProperty("xforms.showXformsSourceTab", "false"));
		map.put("showLayoutXmlTab",
		    Context.getAdministrationService().getGlobalProperty("xforms.showLayoutXmlTab", "false"));
		map.put("showModelXmlTab", Context.getAdministrationService().getGlobalProperty("xforms.showModelXmlTab", "false"));
		map.put("showJavaScriptTab",
		    Context.getAdministrationService().getGlobalProperty("xforms.showJavaScriptTab", "false"));
		map.put("showDesignSurfaceTab",
		    Context.getAdministrationService().getGlobalProperty("xforms.showDesignSurfaceTab", "true"));
		...
		map.put("undoRedoBufferSize",
		    Context.getAdministrationService().getGlobalProperty("xforms.undoRedoBufferSize", "100"));
		map.put("overwriteValidationsOnRefresh",
		    Context.getAdministrationService().getGlobalProperty("xforms.overwriteValidationsOnRefresh", "false"));
		map.put("maintainOrderingOnRefresh",
		    Context.getAdministrationService().getGlobalProperty("xforms.maintainOrderingOnRefresh", "true"));
		...
		map.put("manualWidgetLayout",
		    Context.getAdministrationService().getGlobalProperty("xforms.manualWidgetLayout", "false"));
