...
public class MultimediaServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if ("recentbinary".equals(request.getParameter("action"))) {
			byte[] postData = (byte[]) getSessionData(request, formId,
			    KEY_MULTIMEDIA_POST_DATA + getFieldKey(formId, xpath));
			if (postData != null) {
				response.setContentType(
				    (String) getSessionData(request, formId, KEY_MULTIMEDIA_POST_CONTENT_TYPE + getFieldKey(formId, xpath)));
				...
				if (contentType != null && contentType.trim().length() > 0) {
					...
					if (contentType.contains("video") || contentType.contains("audio"))
						response.setHeader(XformConstants.HTTP_HEADER_CONTENT_DISPOSITION,
						    XformConstants.HTTP_HEADER_CONTENT_DISPOSITION_VALUE + name + "\"");
				}
