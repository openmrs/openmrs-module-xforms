...
public final class Builder {
	 ...
	 * @return - the child element.
	 */
	public static Element getElementByAttributePrefix(Element parent, String attributeName, String attributePrefix, boolean includeObsId){
		for(int i=0; i<parent.getChildCount(); i++){
			if(parent.getType(i) != Element.ELEMENT)
				continue;
			...
			String obsId = child.getAttributeValue(null, "obsId"); //if obs id is already filled, the just look for another node (Problem lists are normally more than one)
			if( (obsId == null || includeObsId) && value != null && value.startsWith(attributePrefix))
				return child;

			child = getElementByAttributePrefix(child,attributeName,attributePrefix, includeObsId);
			if(child != null)
				return child;
		}
	...
	public static String getXform4mDocuments(Document schemaDoc, Document templateDoc) throws Exception {
		...
		List<String> repeatKids = new ArrayList<String>();
		parseTemplate(modelNode,formNode,formNode,bindings,groupNode,problemList,problemListItems);
		parseSchema(schemaDoc.getRootElement(),groupNode,modelNode,xformSchemaNode,bindings,problemList,problemListItems, repeatKids);
	 ...
	 */
	private static void parseSchema(Element rootNode,Element bodyNode, Element modelNode, Element xformSchemaNode, Hashtable bindings, Hashtable<String,String> problemList, Hashtable<String,String> problemListItems, List<String> repeatKids){		
		...
		for (int i = 0; i < numOfEntries; i++) {
			...
			if(name.equalsIgnoreCase(NODE_COMPLEXTYPE) && isUserDefinedSchemaElement(child))
				parseComplexType(child,child.getAttributeValue(null, ATTRIBUTE_NAME),bodyNode,xformSchemaNode,bindings,problemList,problemListItems,repeatControls,modelNode, repeatKids);
			else{
	 ...
	 * @param bindings - a hashtable of node bindings.
	 */
	private static void parseComplexType(Element complexTypeNode,String name, Element bodyNode, Element xformSchemaNode, Hashtable bindings, Hashtable<String,String> problemList, Hashtable<String,String> problemListItems,Hashtable<String,Element> repeatControls, Element modelNode, List<String> repeatKids){
		name = getBindNodeName(name);
		if(name == null)
			return;
		...
		if(nameAttributeValue != null && nameAttributeValue.endsWith("_section")){
			if(!repeatKids.contains(name)){ //As for now, we do not allow nested repeats
				problemList.put(name, name);
		...
		for(int i=0; i<complexTypeNode.getChildCount(); i++){
			...
			if(node.getName().equalsIgnoreCase(NODE_SEQUENCE))
				labelNode = parseSequenceNode(name,node,bodyNode,xformSchemaNode,bindNode,problemList,problemListItems,repeatControls,repeatItem,modelNode, repeatKids);
	...
	private static Element parseSequenceNode(String name,Element sequenceNode,Element bodyNode, Element xformSchemaNode, Element bindingNode , Hashtable<String,String> problemList, Hashtable<String,String> problemListItems,Hashtable<String,Element> repeatControls, boolean repeatItem, Element modelNode, List<String> repeatKids){
		...
		for(int i=0; i<sequenceNode.getChildCount(); i++){
			...
			if(!itemName.equalsIgnoreCase(NODE_VALUE)){
				...
				if(repeatItem && type.endsWith("_section"))
					repeatKids.add(itemName);
			...
			labelNode = parseSequenceValueNode(sequenceNode, name,node,labelNode,bodyNode,bindingNode,problemList,problemListItems,repeatControls);
		}

		if(labelNode == null && repeatKids.contains(name))
			labelNode = parseSequenceValueNode(sequenceNode, name,null,labelNode,bodyNode,bindingNode,problemList,problemListItems,repeatControls);
	 ...
	 */
	private static Element buildSequenceInputControlNode(Element sequenceNode, String name,Element node,String type,Element labelNode, Element bindingNode,Element bodyNode, Hashtable<String,String> problemList, Hashtable<String,String> problemListItems,Hashtable<String,Element> repeatControls){
		if(type != null)
			type = getPrefixedDataType(type);

		if(type != null && !isDataTypeSetPrecisely(bindingNode))
			bindingNode.setAttribute(null, ATTRIBUTE_TYPE, type);

		Element inputNode = bodyNode.createElement(NAMESPACE_XFORMS, null);
		if(node == null){
			assert(type == null);
			inputNode.setName(CONTROL_SELECT1);
			buildSequenceSelectItems(sequenceNode, inputNode);
		}
		else
	...
	private static void buildSequenceSelectItems(Element sequenceNode, Element controlNode){
		for(int i=0; i<sequenceNode.getChildCount(); i++){
			if(sequenceNode.isText(i))
				continue; //ignore text.

			Element node = (Element)sequenceNode.getChild(i);
			String label = node.getAttributeValue(null, ATTRIBUTE_NAME);
			String value = node.getAttributeValue(null, ATTRIBUTE_TYPE);
			...
			itemValNode.setName(NODE_VALUE);	
			itemValNode.addChild(Element.TEXT, value /*value*/ /*binding*/);
	 ...
	 * @return the created label node.
	 */
	private static Element parseSequenceValueNode(Element sequenceNode, String name,Element node, Element labelNode, Element bodyNode,Element bindingNode, Hashtable<String,String> problemList, Hashtable<String,String> problemListItems,Hashtable<String,Element> repeatControls){
		String type = node == null ? null : node.getAttributeValue(null, ATTRIBUTE_TYPE);

		if(type != null || node == null)
			labelNode = buildSequenceInputControlNode(sequenceNode, name,node,type,labelNode,bindingNode,bodyNode,problemList,problemListItems,repeatControls);
		else{
		...
		for (int i = 0; i < numOfEntries; i++) {
			...
			if(tableName != null && columnName != null){
				...
				if(value != null && value.trim().length() > 0){
					...
					velocityEngine.evaluate(velocityContext, w, Builder.class.getName(), value);
					value = w.toString();
	...
	public static String getNewPatientXform()throws Exception {			
		...
		return Builder.fromDoc2String(doc);
	}
